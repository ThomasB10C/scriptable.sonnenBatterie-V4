...
async function createWidget(items) {
    ...
    //data7 = fetchAPIdata( apiURL1, 'string' )
    ...
    if (state.Production_W > 0) { stateProd = "🔆" }
    else if ( hour >= '19' || hour <= '07' ){ stateProd = '🌔' }
    ...
    else if ( state.USOC > 25 )
        line3.textColor = Color.orange()
    else
    ...
    const line10= list.addText(CycleCount + "⤒ ");
